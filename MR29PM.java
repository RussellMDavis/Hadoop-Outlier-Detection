...
public class MR29PM {
	...
	String gridIDStr = ("X"+Integer.toString(gridIDX) + " Y" + Integer.toString(gridIDY));
	...
	int suppIDLowerBound = gridSweep * -1;	
		...
		for(int xIndex = suppIDLowerBound; xIndex <= suppIDUpperBound; xIndex ++){
			for(int yIndex = suppIDLowerBound; xIndex <= suppIDUpperBound; yIndex ++){
				if((yIndex < 1) || (yIndex > 10)){
				}
				else if((xIndex < 1) || (xIndex > 10)){
				}
					...
					gridIDStr = ("X"+Integer.toString(suppIDX) + " Y" + Integer.toString(gridIDY));
				}
			}
		}
	}
...
public static class Reduce extends Reducer<Text, Text, Text, Text> {
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(MR29PM.class);
